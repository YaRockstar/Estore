export const htmlService = {
  ...
  paintOrderData(user, totalPrice) {
    return `
      <h2>Оформление заказа</h2>
        <h3>Данные пользователя</h3>
        <input class="user-name" type="text" placeholder="Имя" value="${user?.name || ''}">
        <input class="user-email" type="text" placeholder="Email (Обязательно)" value="${
          user?.email || ''
        }">
        ...
        <div>Итого: ${totalPrice || 0}₽</div>
        <button data-type="place-order">Оформить заказ</button>
    `;
  },
};
